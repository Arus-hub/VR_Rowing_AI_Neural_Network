...
public class HeadsetMotionGetter : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        for (int c = 0; c < actions.Length; c++)
        {
            ...
            else if (input_bindings[c].EndsWith("trigger"))
            {
                bool triggerPressed = actions[c].ReadValue<float>() > 0.5f; // Assuming the trigger outputs a float between 0 and 1
                outputs[input_bindings[c]] = new Vector4(triggerPressed ? 1 : 0, 0, 0, 0);
            }
            else
            {
